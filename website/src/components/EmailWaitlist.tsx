  ...
  return (
    <section id="waitlist" className="py-20 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="glass-panel-glow rounded-3xl p-8 sm:p-12 text-center relative overflow-hidden border border-brand-cyan/30">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-brand-cyan/10 border border-brand-cyan/30 text-brand-cyan text-xs font-semibold mb-6">
            ...
            <span>Join 1,200+ Engineers in the Superpower Community</span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="mt-8 max-w-md mx-auto">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3.5 bg-[#0a0d16] border border-white/15 focus:border-brand-cyan rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-cyan/20 transition-all font-sans"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={status === 'loading'}
                className="px-6 py-3.5 rounded-xl bg-gradient-to-r from-brand-cyan to-brand-blue hover:from-brand-cyan/90 hover:to-brand-blue/90 text-slate-950 font-bold text-sm shadow-lg shadow-brand-cyan/25 flex items-center justify-center gap-2 transition-all disabled:opacity-50"
              >
                {status === 'loading' ? (
                  <>
            ...
            {/* Status Messages */}
            {status === 'success' && (
              <div className="mt-4 flex items-center justify-center gap-2 text-xs text-brand-success bg-brand-success/10 border border-brand-success/30 rounded-lg p-2.5 animate-fade-in">
            ...
            {status === 'error' && (
              <div className="mt-4 flex items-center justify-center gap-2 text-xs text-brand-danger bg-brand-danger/10 border border-brand-danger/30 rounded-lg p-2.5 animate-fade-in">
            ...
            <p className="mt-3 text-[11px] text-slate-500">
              🔒 Zero spam. Stored securely in MongoDB. Unsubscribe anytime in 1 click.
            </p>
          </form>
        </div>
      </div>
